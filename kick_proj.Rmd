...
#remove projects with goal under 1$
  ...
  mutate(bin_status=ifelse(status=='successful',1,0))
...
13163/(13163+63281) #17% of failed projects have no backers!!! ###############################################report this
...
ks %>%
  ...
  theme_linedraw()
  #consider scaling wiht custom color gradient
...
ks %>%
  ...
  summarize(n(),
            ...
            succ_rate=sum(status=='successful')/n()) %>%
  ggplot() +
  geom_point(aes(x=name_wrd,y=succ_rate,size=avg_goal,color=avg_backers)) +
  theme_linedraw()
  #consider scaling wiht custom color gradient
...
logit.overall = glm(status ~backers_count + country + usd_pledged + sub_category + main_category + goal_usd + duration + launch_month + launch_dom + +launch_dow + launch_hour + name_len + name_wrd,
                    family = 'binomial',
                    data = ks)
