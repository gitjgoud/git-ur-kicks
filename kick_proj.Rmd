...
```{r log_model}
# ---- Second Attempt ----
#selected variables expected to have highest impact
...
predict(logit.overall, newdata) #This gives us the log odds; but we want
                                #the probabilities.
                                #we know these are log odds because they are negative

#Using the formula to convert to probabilities:
exp(predict(logit.overall, newdata))/(1 + exp(predict(logit.overall, newdata)))
#all positive means they could be probabilities if we lacked contect
