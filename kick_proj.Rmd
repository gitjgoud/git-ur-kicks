...
ks %>% slice_min(backers_count,prop = .995) %>% min()
...
ks %>% slice_min(backers_count,prop = .995) %>%
  ggplot(aes(y=backers_count,x=status)) +
...
ks %>% slice_min(goal_usd,prop = .995) %>%
  ggplot(aes(x=goal_usd)) +
  geom_histogram(aes(fill=status),alpha=.7,position=position_dodge(20))
...
ks %>% slice_min(avg_plg.bkr,prop = .995) %>%
  ggplot(aes(x=avg_plg.bkr)) +
  geom_histogram(aes(fill=status),alpha=.7,position=position_dodge(20))
...
#revisit after applying other slices, as this may
...
ks %>% 
  slice_min(backers_count,prop = .995) %>%
  slice_min(goal_usd,prop = .995) %>%
  slice_min(avg_plg.bkr,prop = .995) %>%
