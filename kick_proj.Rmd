...
library(ggplot2)
library(gridExtra)
library(plyr) #needed to mutate using round_any, REMOVE AFTER OR IT WILL BREAK PLOTS

```

```{r Load Data & Filter/Mutate}
#### Load data from file ------------------------------------------------------
...
ks <- ks_b %>% 
  ...
  #bins avb_plg.bkr by nearest 10
  mutate(avg_plg.bkr.bin=round_any(avg_plg.bkr,10)) %>%           #requires plyr, which may mess with dplyr some - rec removing after runnng this chunk
  #name counts
...
#unload plyr, reload dplyr to fix some issues with masked functions
...
```{r export clean ks df for app}

write.csv(ks,file='clean_kickstarter_data.csv',row.names = F)
...
- advanced methods were too challenging/time consuming to implement {https://www.itl.nist.gov/div898/handbook/eda/section3/eda35h.htm},etc
- evaluated using IQR method when possible (3x) {https://machinelearningmastery.com/how-to-use-statistics-to-identify-outliers-in-data/}
...
ggsave("www/name1.JPG",plot=name1)
...
ggsave("www/name2.JPG",plot=name2)
...
count3 <-country %>%
  ggplot(aes(x=reorder(country,succ_rate),y=avg_plg)) +
  geom_hline(yintercept = mean(ks$avg_plg.bkr),linetype=3)+
  geom_point() +
  theme_linedraw() +
...
count4<-country %>%
  ggplot(aes(x=reorder(country,succ_rate),y=avg_bkrs)) +
  geom_hline(yintercept=mean(ks$backers_count),linetype=3) +
  geom_point() +
  theme_linedraw() +
...
country %>%
  ...
  geom_hline(yintercept=mean(ks$goal_usd),linetype=3) +
  geom_point() +
  theme_linedraw() +
  labs(title='Country vs Average Backer Count',x='',y='Average Num Backers',
       size="Succ Rate", color="Average Goal") 
...
#USE THIS
con2<-grid.arrange(count1,count2,count3,count4,nrow=2)
ggsave("www/con2.JPG",plot=con2)
...
#USE THIS
bkr1<-grid.arrange(bkr1,bkr2,ncol=2)
ggsave("www/bkr1.JPG",plot=bkr1)
...
#USE THIS
bkr2<-grid.arrange(bkr3,bkr4,nrow=2)
ggsave("www/bkr2.JPG",plot=bkr2)
...
ks %>%
  transmute(round_any(avg_plg.bkr,10))

#USE THIS
bkr3<-ks %>%
  group_by(avg_plg.bkr.bin) %>%
  summarize(ct_succ=sum(status=='successful'),
            ...
            avg_goal=mean(goal_usd),
            projects=n()) %>%
  ggplot(aes(x=avg_plg.bkr.bin,y=pct_succ,size=projects,col=avg_goal)) +
  geom_point() +
  geom_hline(yintercept = sum(ks$status=='successful')/sum((ks$status=='successful'|ks$status=='failed')),linetype=3)+
  scale_x_continuous(limits=c(0,700),breaks=seq(0,700,by=50)) +
  theme_linedraw() +
  theme(panel.grid.minor=element_blank()) +
  labs(title='Average Pledge Trends',x='Average Pledge (USD)',y='Percent success') +
  ylim(0,1)

ggsave("www/bkr3.JPG",plot=bkr3)
...
fund2<-grid.arrange(goal1,goal2,goal3,nrow=3)
ggsave("www/fund2.JPG",plot=fund2)
...
ggsave("www/fund1.JPG",plot=fund1)
...
ggsave("www/fund3.JPG",plot=fund3)
...
cat2<-main_cat %>%
  ...
  theme_linedraw() +
  scale_color_gradient(low='grey',high='firebrick4') +
  labs(x='',y='Percent Success',title='Detailed Category (aka MainCategory) Success') +
  theme(axis.text.x = element_blank(),
        axis.ticks.x = element_blank(),
        panel.grid.major.x = element_blank())

ggsave("www/cat2.JPG",plot=cat2)
...
ks %>%
  ...
  summarize(ct_succ=sum(status=='successful'),
            ...
            avg_goal=mean(goal_usd),
            projects=n()) %>%
  ggplot(aes(x=dead_month,y=pct_succ,size=projects,col=avg_bkr)) +
  geom_point() +
  geom_hline(yintercept = sum(ks$status=='successful')/sum((ks$status=='successful'|ks$status=='failed')),linetype=3)+
  scale_x_continuous(limits=c(1,12),breaks=seq(1,12)) +
  ylim(c(0,1)) +
  theme_linedraw() +
  theme(panel.grid.minor=element_blank()) +
  labs(title='Deadline Month Tends',x='Month',y='Percent success')+
  scale_color_gradient(high='grey',low='firebrick4')
...
ks %>%
  ...
  summarize(ct_succ=sum(status=='successful'),
            ...
            avg_goal=mean(goal_usd),
            projects=n()) %>%
  ggplot(aes(x=dead_dom,y=pct_succ,size=projects,col=avg_bkr)) +
  geom_point() +
  geom_hline(yintercept = sum(ks$status=='successful')/sum((ks$status=='successful'|ks$status=='failed')),linetype=3)+
  scale_x_continuous(limits=c(1,31),breaks=seq(1,31)) +
  ylim(c(0,1)) +
  theme_linedraw() +
  theme(panel.grid.minor=element_blank()) +
  labs(title='Deadline Day Tends - Day of Month',x='Day of Month',y='Percent success')+
  scale_color_gradient(high='grey',low='firebrick4')
...
ks %>%
  ...
  summarize(ct_succ=sum(status=='successful'),
            ...
            avg_goal=mean(goal_usd),
            projects=n()) %>%
  ggplot(aes(x=dead_dow,y=pct_succ,size=projects,col=avg_bkr)) +
  geom_point() +
  geom_hline(yintercept = sum(ks$status=='successful')/sum((ks$status=='successful'|ks$status=='failed')),linetype=3)+
  scale_x_continuous(limits=c(1,7),breaks=seq(1,7)) +
  ylim(c(0,1)) +
  theme_linedraw() +
  theme(panel.grid.minor=element_blank()) +
  labs(title='Deadline Day Tends - Day of Week',x='Day of week',y='Percent success')+
  scale_color_gradient(high='grey',low='firebrick4')
...
ks %>%
  ...
  summarize(ct_succ=sum(status=='successful'),
            ...
            avg_goal=mean(goal_usd),
            projects=n()) %>%
  ggplot(aes(x=dead_hour,y=pct_succ,size=projects,col=avg_bkr)) +
  geom_point() +
  geom_hline(yintercept = sum(ks$status=='successful')/sum((ks$status=='successful'|ks$status=='failed')),linetype=3)+
  scale_x_continuous(sec.axis = sec_axis(~ .-5,name="Hour (EST)"),
                     limits=c(0,23),breaks=seq(0,23),) +
  ...
  theme_linedraw() +
  theme(panel.grid.minor=element_blank()) +
  labs(title='Deadline Hour Trends',x='Hour (UTC)',y='Percent success')+
  scale_color_gradient(high='grey',low='firebrick4')
...
dur1<-ks %>%
  ...
  summarize(ct_succ=sum(status=='successful'),
            ...
            avg_goal=mean(goal_usd),
            projects=n()) %>%
  ggplot(aes(x=duration,y=pct_succ,size=projects,col=avg_goal)) +
  geom_point() +
  geom_hline(yintercept = sum(ks$status=='successful')/sum((ks$status=='successful'|ks$status=='failed')),linetype=3)+
  ...
  theme_linedraw() +
  theme(panel.grid.minor=element_blank()) +
  labs(title='Project Duration',x='days',y='Percent success')

ggsave("www/dur1.JPG",plot=dur1)
...
```{r Creator Experience}

#USE THIS
ctr1 <- ks %>%
  group_by(creator_id) %>%
  mutate(num_projects.creator=n()) %>%
  ungroup() %>%
  group_by(num_projects.creator) %>%
  summarize(ct_succ=sum(status=='successful'),
            ct_fail=sum(status=='failed'),
            pct_succ=ct_succ/n(),
            avg_bkr=mean(backers_count),
            avg_goal=mean(goal_usd),
            count=n()) %>%
  ggplot(aes(x=num_projects.creator,y=pct_succ,size=count,col=avg_goal)) +
  geom_point() +
  geom_hline(yintercept = sum(ks$status=='successful')/sum((ks$status=='successful'|ks$status=='failed')),linetype=3)+
  scale_x_continuous(limits=c(1,60),breaks=c(1,seq(5,60,by=5))) +
  ylim(c(0,1)) +
  theme_linedraw() +
  theme(panel.grid.minor=element_blank()) +
  labs(title='Creator Experience',x='Number of Projects Attempted',y='Percent success')

ggsave("www/ctr1.JPG",plot=ctr1)
...
## Build a model
- First few attempts didn't work
- Spent a day attempting to remove outliers from data and identify important factors
- Got a working model with 79.5% prediction accuracy using original dataset
- Needs work

```{r log_model w/ export dataset - testing for app}
#this should be the same as below but code uses exported CSV for data
#tying to figure out how to get shiny app to use this 
...
scatter.smooth(logit.overall.app$fit,
...
exp(logit.overall.app$coefficients)

summary(logit.overall.app)
...
predict(logit.overall.app, newdata,type='response')


#-------
status.predicted = round(logit.overall.app$fitted.values)
length(status.predicted)
length(ks_app$status_binary)

table(truth = ks_app$status_binary, prediction = status.predicted)
# truth      0      1
#     0  45694  29534
#     1  11336 113095
(45694+113095)/(113095+11336+29534+45694)
#79.5% accuracy! No time to tune - GOING WITH IT :)))
...
```{r log_model}
# ---- Second Attempt ----
#selected variables expected to have highest impact (reducing until model converges)
# Warning message:
# glm.fit: fitted probabilities numerically 0 or 1 occurred 
#no error with these variables selected

logit.overall = glm(status_binary ~ avg_plg.bkr + sub_category + goal_usd + duration + launch_hour + name_len,
                    family = 'binomial',
                    data = ks)

redi.plot <-scatter.smooth(logit.overall$fit,
               residuals(logit.overall, type = "deviance"),
               lpars = list(col = "red"),
               xlab = "Fitted Probabilities",
               ylab = "Deviance Residual Values",
...
#not working, giving up on this one and using ss
ggsave("www/residuals.JPG",plot=redi.plot)
ggsave("RegressionPlot.jpeg",plot=redi.plot)
...
newdata = with(ks, data.frame(avg_plg.bkr=mean(avg_plg.bkr),
                              sub_category='technology',
                              goal_usd=3000,
                              duration=mean(duration),
                              launch_hour=median(launch_hour),
...
#-------
status.predicted = round(logit.overall$fitted.values)
length(status.predicted)
length(ks$status_binary)

table(truth = ks$status_binary, prediction = status.predicted)
# truth      0      1
#     0  45694  29534
#     1  11336 113095
(45694+113095)/(113095+11336+29534+45694)
...
```{r IGNORE log_model_reduced_goal}


ks.r <- ks %>% filter(goal_usd<50000)

logit.overall.r = glm(status_binary ~backers_count + sub_category + goal_usd + duration + launch_hour + name_len,
                    family = 'binomial',
                    data = ks.r)
